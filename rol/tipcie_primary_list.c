/*****************************************************************
 * 
 * tipcie_primary_list.c - "Primary" Readout list routines for tipcie_primary
 *
 * Usage:
 *
 *    #include "tipcie_primary_list.c"
 *
...
#define ROL_NAME__ "TIPCIE_PRIMARY"
...
#ifndef INIT_NAME
#warn "INIT_NAME undefined. Setting to tipcie_primary_list__init"
#define INIT_NAME tipcie_primary_list__init
#endif
#include <stdio.h>
#include <rol.h>
#include "jvme.h"
#include <TIPCIE_PRIMARY_source.h>
#include "tiLib.h"
...
/* Asynchronous (to tipcie_primary rol) trigger routine, connects to rocTrigger */
void asyncTrigger();
...
static void __prestart()
{
  ...
  *(rol->nevents) = 0;
  ...
  TIPCIE_PRIMARY_INIT;
  CTRIGRSS(TIPCIE_PRIMARY,1,usrtrig,usrtrig_done);
  CRTTYPE(1,TIPCIE_PRIMARY,1);
...
/*     { */
/* /\*       chuck=1; *\/ */
/*       printf("tipcie_primary_list End: %d events left on vmeOUT queue (will now de-queue them)\n",rem_count); */
/*       /\* This wont work without a secondary readout list (will crash EB or hang the ROC) *\/ */
...
/*   else */
/*     { */
/*       printf("tipcie_primary_list End: vmeOUT queue Empty\n"); */
/*     } */
...
/* 	      crateShmSetData(event_number, shmData, len); */
	...
	{
	  printf("tipcie_primary_list: ERROR rol->dabufp is NULL -- Event lost\n");
	}
